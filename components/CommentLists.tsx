	...
	return (
		<div className="max-h-40 scrollbar-hidden hover:scrollbar overflow-y-scroll overflow-x-hidden my-4">
			{confession.comments.map((comment: TComment, index: number) => (
				<div key={comment._id} className="w-full flex items-start space-x-3 p-4 pb-0 shadow rounded-lg" ref={index === confession.comments.length - 1 ? latestCommentRef : null}>
